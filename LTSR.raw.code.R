...
.make_sample_metadata <- function(obs_tbl, colSample, colVarCats = c(), colVarNums = c(), extra_term = NULL) {
  ...
  print(colnames(obs_tbl))
  print(colSample)
  print(colVarCats)
  print(colVarNums)
  print(extra_vars)
  print(c(colSample, colVarCats, colVarNums, extra_vars))
  print(c(colSample, colVarCats, colVarNums, extra_vars))
  metadata_tbl <- obs_tbl %>%
    dplyr::select(one_of(unique(c(colSample, colVarCats, colVarNums, extra_vars)))) %>%
    unique()
  metadata_tbl
}
...
.make_formula <- function(colSample, colCelltype, colVarCats, colVarNums, extra_term = NULL) {
  ...
  # 先定义一个生成交互项的辅助函数，用于指定两两分类变量的交互
  generate_interaction_terms <- function(var_list) {
    combn(var_list, 2, \(x) sprintf("(1|%s:%s:Celltype)", x[1], x[2]), simplify = FALSE)
  }
  
  # 使用辅助函数生成所需的交互项
  interaction_terms <- generate_interaction_terms(colVarCats)
  terms <- c(
    colVarNums,
    sprintf("(1|%s)", c(colSample, colVarCats)),
    sprintf("(%s-1|Celltype)", colVarNums),
    sprintf("(1|%s:Celltype)", c(colSample, colVarCats))#,
    #interaction_terms  #加入交叉因素，但是拟合有问题
  )
...
.getCondValLtsr <- function(ranef_tbl, vars, celltypes = NULL, references = NULL) {
    ...
    cat_ranef_tbl <- bind_rows(
      lapply(names(vars), function(vname) {
        if (vname %in% names(references)) {
          ...
          full_join(
            cat_ranef_tbl %>% dplyr::filter(grpvar == vname, grpval != ref),
            cat_ranef_tbl %>% dplyr::filter(grpvar == vname, grpval == ref) %>% select(grpvar, Celltype, condval, condsd),
          ...
          return(cat_ranef_tbl %>% dplyr::filter(grpvar == vname))
        }
      })
    )
  }
  ...
  if (!is.null(celltypes)) ranef_tbl <- ranef_tbl %>% dplyr::filter(Celltype %in% celltypes)
